...
class CartItemController extends Controller
{
        ...
        if (empty($user_cart)) {
            ...
            $all_user_cart_items = DB::table("carts")
                ...
                ->select(
                    ...
                    "carts.user_id",
                    "cart_items.type",
                    "cart_items.charge_value",
                    "vendor_services.id as vendor_service_id",
                    DB::raw(
                        "CONCAT('https://digifarmer.agrosahas.co/farmerapp/public/storage/vendor_services/', vendor_services.image) AS image"
                    ),
                    "vendor_services.name",
                    "vendor_services.price_unit",
                    "vendor_services.price",
                    "vendor_services.charge",
                    "cart_items.total_cost"
                )
                ->get();
            ...
            $response = [
                ...
                "data" => [
                    ...
                    "total-cart-quantity" => $all_user_cart_items->sum(
                        "quantity"
                    ),
                    "total-grand_amount" => $all_user_cart_items->sum(
                        "total_cost"
                    ),
    ...
    public function add_product_to_cart(Request $request, $id)
    {
        ...
        if ($existing_user_cart) {
            ...
            if (
                CartItem::where("cart_id", $existing_user_cart->id)
                    ->where("vendor_service_id", $id)
                    ->first()
            ) {
                $response = [
                    "success" => false,
    ...
    public function increase_quantity(Request $request, $id)
    {
        ...
        if (
            $vendor_service->sub_category->category->name == "Rent" &&
            !empty($vendor_service->charge)
        ) {
        ...
        if ($product->quantity > $vendor_service->stock_amount - 1) {
            ...
            if (
                CartItem::where("id", $id)->update([
                    ...
                    "total_cost" => DB::raw("quantity * '$price'"),
                ]) == 1
            ) {
                $response = [
                    "success" => true,
                    "message" =>
                        "Product quantity has been increased successfully",
                ];
                return response()->json($response, 200);
            } else {
                $response = [
    ...
    public function increase_charge_value(Request $request, $id)
    {
        ...
        if (!($product->type == "rent")) {
            $response = [
                "success" => false,
                "message" =>
                    "Charge value is only increased for rent items on hire",
            ];
            return response()->json($response, 400);
        }
        ...
        if ($vendor_service->charge_frequency == "per piece") {
            ...
            CartItem::where("id", $id)->update([
                "charge_value" => DB::raw("charge_value+1"),
                "total_cost" => DB::raw(
                    "charge_value * '$vendor_service->charge'"
                ),
            ]);
            $response = [
                "success" => true,
                "message" =>
                    "Product charge period has been increased successfully",
            ];
            return response()->json($response, 200);
        }
    }
    ...
    public function reduce_charge_value(Request $request, $id)
    {
        ...
        if ($vendor_service->charge_frequency == "per piece") {
            ...
            $response = [
                "success" => true,
                "message" =>
                    "Product charge period has been reduced successfully",
            ];
            return response()->json($response, 200);
        }
    }
